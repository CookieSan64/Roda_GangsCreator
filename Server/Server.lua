...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    Wait(100)
    TriggerClientEvent('chat:addSuggestion', -1, '/setgang', 'Pour attribuer un groupe à un joueur', {
        { name = 'playerId', help = 'ID du joueur' },
        { name = 'gangName', help = 'Nom du groupe' },
        { name = 'gangRank', help = 'Rang du groupe' }
    })
...
RegisterCommand('recrutemember', function(source)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
AddEventHandler('Roda_GangsCreator:server:UpdatePoints', function()
    ...
    print('^3[Roda_GangsCreator]^0 - ^2 Actualisation des Groupes. ^0')
end)
